...
const computePath = (route: string): string => {
    ...
    if (CLEAN_PATH_PREFIXES.some((prefix) => route === prefix || route.startsWith(prefix + '/'))) {
        return route;
    }
...
const computeSubPath = (route: IRoute, subroute: IRouteItem): string => {
    ...
    if (route.name === 'workspace') {
        ...
        if (subroute.name === 'my-projects') return '/apprenticeship' + resolved;
...
const computeInPath = (inroute: IInRoute): string => {
    ...
    if (route?.name === 'workspace' && inroute.parent) {
        if (inroute.parent === 'my-hackathons') return '/hackathon' + inroute.url + resolved;
        if (inroute.parent === 'my-challenges') return '/challenge' + inroute.url + resolved;
        if (inroute.parent === 'my-projects') return '/apprenticeship' + inroute.url + resolved;
        return route.url + (route.subroutes?.find((m) => m.name === inroute.parent)?.url ?? '') + inroute.url + resolved;
    }
...
const inRoute = (payload: { route: string, name: string, params?: Array<IRouteParam> }): string => {
    ...
    const slugParam = params?.find((p) => (p.type === 'url' && (p.name === 'id' || p.name === 'slug' || p.name === 'details') && p.value)) as IRouteParam | undefined;
