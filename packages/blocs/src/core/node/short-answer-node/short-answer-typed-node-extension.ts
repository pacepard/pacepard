...
function nodeNameForType(inputType: InputType): string {
    return (
        'shortAnswer' + inputType.charAt(0).toUpperCase() + inputType.slice(1)
    );
}
...
export function createShortAnswerTypedNode(
    ...
    return Node.create<ShortAnswerTypedNodeOptions>({
        ...
        parseHTML() {
            return [
                {
                    tag: `div[data-type="short-answer-node"][data-node-name="${name}"]`,
                },
            ];
        },
        ...
        addNodeView() {
            return ReactNodeViewRenderer(ShortAnswerNodeComponent, {
                stopEvent: ({ event }) => {
                    ...
                    if (!el) return false;
                    if (el.closest('.short-answer-node__input-wrap'))
                        return true;
                    return false;
                },
            });
        ...
        addProseMirrorPlugins() {
            ...
            const normalizationPlugin = new Plugin({
                ...
                appendTransaction(_transactions, _oldState, newState) {
                    ...
                    newState.doc.descendants((node, pos) => {
                        ...
                        if (fixed != null)
                            replacements.push({ pos, node, fixed });
                    });
            ...
            const keymapPlugin = keymap({
                Backspace: (state, dispatch) => {
                    ...
                    if (parentNode.type.name !== shortAnswerTypeName)
                        return false;
            ...
            return {
                [commandName]:
                    ...
                    ({
                        ...
                        state: { schema: { nodes: Record<string, unknown> } };
                    }) => {
                        const schema = state.schema;
                        if (!schema.nodes.heading) return false;
                        const { title: _title, ...restAttrs } =
                            attrs as Partial<ShortAnswerAttrs> & {
                                title?: string;
                            };
                        const titleText = _title ?? 'Question';
                        const inputTypeName = schema.nodes[
