...
export function createShikiHighlightPlugin(options: ShikiPluginOptions) {
  ...
  const lazyParser = (
    ...
    if (!highlighter) {
      const initPromise =
        globalThisForShiki[SHIKI_HIGHLIGHTER_KEY] ?? createHighlighter()
      globalThisForShiki[SHIKI_HIGHLIGHTER_KEY] = initPromise
      const onFulfilled = (h: ShikiHighlighter) => {
        highlighter = h
        return runParser(parserOptions)
      }
      const resultPromise = (initPromise as Promise<ShikiHighlighter>).then(
        onFulfilled as (value: ShikiHighlighter) => void | PromiseLike<void>
  ...
  function runParser(parserOptions: {
    ...
    const loadLang =
      !highlighter.getLoadedLanguages().includes(language)
        ? highlighter.loadLanguage(language as Parameters<ShikiHighlighter["loadLanguage"]>[0])
