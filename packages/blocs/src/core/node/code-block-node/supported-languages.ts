 ...
 * Used for language picker, input rules (```lang), and Shiki plugin resolution.
 */
...
/**
 * Resolve a language name or alias to the canonical language id.
 */
export function getLanguageId(languageName: string): string | undefined {
  const normalized = languageName.trim().toLowerCase()
  if (!normalized) return undefined
  const entry = Object.entries(supportedLanguages).find(
    ([id, { aliases }]) => id === normalized || aliases?.includes(normalized)
  )
  return entry?.[0]
}
