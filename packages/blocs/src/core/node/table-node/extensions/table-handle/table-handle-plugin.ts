...
import { isValidPosition } from '@/utils/base-helper';
import { createTableDragImage } from '@/core/node/table-node/extensions/table-handle/helpers/create-image';
...
class TableHandleView implements PluginView {
    ...
    private viewMousedownHandler = (event: MouseEvent) => {
        ...
        for (let d = $pos.depth; d >= 0; d--) {
            ...
            if (
                !inTableCell &&
                (node.type === nodes.tableCell ||
                    node.type === nodes.tableHeader)
            ) {
                inTableCell = true;
            }
    ...
    private _handleMouseMoveNow(event: MouseEvent) {
        ...
        const wrapper = safeClosest<HTMLElement>(
            around.domNode,
            '.tableWrapper',
        );
        ...
        if (around.type === 'wrapper') {
            ...
            this.state = {
                ...
                widgetContainer,
                colIndex: cursorBeyondRightOrBottom
                    ? undefined
                    : this.state?.colIndex,
                rowIndex: cursorBeyondRightOrBottom
                    ? undefined
                    : this.state?.rowIndex,
            ...
            const cellRect = (
                around.domNode as HTMLElement
            ).getBoundingClientRect();
            const lastRowIndex = blockInfo.node.content.childCount - 1;
            const lastColIndex =
                (blockInfo.node.content.firstChild?.content.childCount ?? 0) -
                1;
    ...
    dropHandler = () => {
        ...
        if (
            (draggingState.draggedCellOrientation === 'row' &&
                rowIndex === undefined) ||
            (draggingState.draggedCellOrientation === 'col' &&
                colIndex === undefined)
        ) {
            throw new Error(
    ...
    update(view: EditorView): void {
        ...
        const { height: rowCount, width: colCount } = TableMap.get(
            tableInfo.node,
        );
    ...
    destroy(): void {
        ...
        window.removeEventListener(
            'mouseup',
            this.mouseUpHandler as EventListener,
        );
        this.editorView.dom.removeEventListener(
            'mousedown',
...
export function TableHandlePlugin(
    ...
    return new Plugin({
        ...
        view: (editorView) => {
            tableHandleView = new TableHandleView(
                editor,
                editorView,
                emitUpdate,
            );
        ...
        props: {
            decorations: (state) => {
                ...
                const newIndex =
                    tableHandleView.state.draggingState
...
const tableDragStart = (
    ...
    if (editor.state.selection instanceof CellSelection) {
        const safeSel = TextSelection.near(
            editor.state.doc.resolve(blockPos),
            1,
        );
        editor.view.dispatch(editor.state.tr.setSelection(safeSel));
    }

    const dragImage = createTableDragImage(
        editor,
        orientation,
        index,
        blockPos,
    ...
    const initialOffset = referencePosCell
        ? (orientation === 'col'
              ? referencePosCell.left
              : referencePosCell.top) - mousePos
        : 0;
