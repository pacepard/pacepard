...
// --- Lib ---
import { isExtensionAvailable, isValidPosition } from '@/utils/base-helper';
...
// --- Icons ---
import { TableHeaderRowIcon } from '@/core/icons/table-header-row-icon';
import { TableHeaderColumnIcon } from '@/core/icons/table-header-column-icon';
...
export function useTableHeaderRowColumn(config: UseTableHeaderRowColumnConfig) {
    ...
    if (editor?.state.selection instanceof CellSelection) {
        ...
        if (rowsOrCols) {
            ...
            isActive =
                rowsOrCols.cells[secondIndex]?.node?.type.name ===
                    'tableHeader' || false;
        }
    }

    const handleToggle = useCallback(() => {
        const success = toggleTableHeader({
            editor,
            index,
            orientation,
            tablePos,
    ...
    const label =
        tableHeaderRowColumnLabels[selectionType?.orientation || 'row'];
