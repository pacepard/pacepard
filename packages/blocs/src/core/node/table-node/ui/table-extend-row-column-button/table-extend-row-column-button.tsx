...
import { useTableHandleState } from '@/core/node/table-node/hooks/use-table-handle-state';
...
// --- Icons ---
import { PlusSmallIcon } from '@/core/icons/plus-small-icon';

// --- Internal ---
import { useTableExtendRowColumnButtonsPositioning } from '@/core/node/table-node/ui/table-extend-row-column-button/use-table-extend-row-column';
...
> = ({
    ...
    useEffect(() => {
        ...
        const handleMove = (ev: MouseEvent) => {
            ...
            const cellSize = isRowOrientation
                ? EMPTY_CELL_HEIGHT
                : EMPTY_CELL_WIDTH;
...
> = ({ editor: providedEditor, onMouseDown, onMouseUp }) => {
    const { editor } = usePacepardEditor(providedEditor);
    const state = useTableHandleState({ editor });
    const { columnButton, rowButton } =
        useTableExtendRowColumnButtonsPositioning(
