...
import { useToc } from '@/core/node/toc-node/context/toc-context';
...
export function TocNodeComponent(props: NodeViewProps) {
    ...
    return (
        <NodeViewWrapper {...wrapperProps}>
            {showTitle && (
                <div className="tiptap-table-of-contents-title">
                    Table of contents
                </div>
            )}
            <nav
            ...
            >
                {flatTocList.map((node, index) => (
                    <a
                        key={
                            node.item.id ?? `${index}-${node.item.textContent}`
                        }
                        href={`#${node.item.id}`}
                        ...
                        style={
                            { '--toc-depth': node.depth } as React.CSSProperties
                        }
