...
function createFormInputExtension(inputType: InputType) {
    ...
    const commandName = ('insertFormInput' +
        (inputType === 'text'
            ? 'Text'
            : inputType.charAt(0).toUpperCase() + inputType.slice(1))) as
        | 'insertFormInputText'
...
declare module '@tiptap/core' {
    interface Commands<ReturnType> {
        ...
        formInputUrl: {
            insertFormInputUrl: (attrs?: Partial<FormInputAttrs>) => ReturnType;
        };
        formInputTel: {
            insertFormInputTel: (attrs?: Partial<FormInputAttrs>) => ReturnType;
        };
    }
}
