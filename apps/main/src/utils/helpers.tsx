...
const equalLength = (id: string, childId: string, len?: number) => {
    ...
    const height = heigthList.length > 0 ? Math.max(...heigthList) : 0; // get the highest length;

    for (let i = 0; i < items.length; i++) {
        const item = items[i];
        if (item) {
            const itemHeight = ($(item) as JQuery<HTMLElement>).height();
            if (itemHeight !== undefined && Math.floor(itemHeight) !== height) {
...
const capitalizeWord = (value: string): string => {
    ...
    if (value.includes('-')) {
        ...
        for (var i = 0; i < split.length; i++) {
            const item = split[i];
            if (item) {
                split[i] = item.charAt(0).toUpperCase() + item.slice(1);
            } else {
                split[i] = '';
            }
        }
