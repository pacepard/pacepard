            ...
            if (isBlocsFile && hasAtImports) {
                ...
                let transformedCode = code.replace(
                    ...
                    (match, importPath) => {
                        ...
                        const actualRelativePath = path.relative(
                            fileDir,
                            actualPath,
                        );

                        // Normalize the path for use in imports (use forward slashes)
                        const normalizedPath = actualRelativePath
                            .split(path.sep)
                            .join('/');
        ...
        resolveId(id, importer) {
            ...
            if (
                ...
                if (normalizedImporter.includes('packages/blocs/src')) {
                    ...
                    // Try as directory
                    if (
                        fs.existsSync(correctPath) &&
                        fs.statSync(correctPath).isDirectory()
                    ) {
                        for (const ext of extensions) {
                            const indexPath = path.join(
                                correctPath,
