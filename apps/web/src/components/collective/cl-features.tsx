...
const topTabs = [
    {
        ...
        description: 'One searchable home for your specs, OKRs, and legacy docs',
        icon: FileText,
    ...
    },
    {
        ...
        description: 'Build effective roadmaps your whole team can shape, and ship',
        icon: Map,
    ...
    },
    {
        ...
        description: 'Track every sprint, bug, launch across all your tools & teams',
        icon: Bug,
...
export default function ClFeatures() {
    ...
    const [contentTab, setContentTab] = useState<'policies' | 'company'>('policies');
            ...
            <div className="flex flex-row flex-wrap items-center justify-between gap-8 lg:gap-16">
                ...
                <Tabs defaultValue="knowledge-base" className="w-full">
                    <TabsList className="mb-8 grid h-auto w-full grid-cols-1 gap-4 bg-transparent p-0 sm:grid-cols-2 lg:grid-cols-4">
                        {topTabs.map(({ id, label, description, icon: Icon, iconColor }) => (
                            <TabsTrigger
                                key={id}
                                value={id}
                                className={cn(
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            ...
                                            <nav className="space-y-0.5">
                                                {teamspaces.map(({ id, label, icon: Icon }) => (
                                                    <button
                                                        key={id}
                                                        type="button"
                                                        onClick={() => setActiveTeamspace(id)}
                                                        className={cn(
                                            ...
                                            {!imageError ? (
                                                ...
                                                <div className="flex h-full w-full flex-col items-center justify-center gap-4 p-8 text-center">
                                                    ...
                                                    <div className="flex w-full max-w-md flex-col gap-4 rounded-lg border border-dashed border-border p-6">
                                                        ...
                                                        <Tabs value={contentTab} onValueChange={(v) => setContentTab(v as 'policies' | 'company')} className="w-full">
                                                            ...
                                                            <TabsContent value="company" className="mt-4 space-y-2">
                                                                {companyItems.map(({ label, icon }) => (
                                                                    <div key={label} className="flex items-center gap-2 text-sm text-muted-foreground">
                                                                        <span>{icon}</span>
                                                                        {label}
                                                                    </div>
                                    ...
                                    <div className="hidden w-72 shrink-0 border-l border-border p-4 lg:block">
                                        <div className="rounded-xl border border-border bg-muted/30 p-4 shadow-sm">
                                            <h4 className="mb-2 flex items-center gap-2 font-semibold text-foreground">
                                                <span className="flex size-6 items-center justify-center rounded bg-primary/20 text-primary text-xs">✨</span>
                                                Career goals at Acme
                                            </h4>
                                            <p className="mb-3 text-xs text-muted-foreground">Today 10:44 AM</p>
                                            <p className="mb-2 text-sm text-muted-foreground">
                                                Where can I learn more about career goals at Acme Labs?
                                            </p>
                                            <p className="mb-4 text-sm text-foreground">
                                                You can learn more about career goals at Acme labs by visiting the Career Progression page.
                                            </p>
                                            <p className="mb-2 text-xs font-medium text-muted-foreground">15 results</p>
                                            <ul className="space-y-1 text-sm text-primary">
                                                <li>Levels at Acme</li>
                                                <li>Teams & Org Chart</li>
                                                <li>Performance Reviews 2025</li>
                                                <li>More ↓</li>
                                            </ul>
                                            <div className="mt-4 flex gap-2">
                                                <button type="button" className="rounded-md border border-border bg-background px-2 py-1 text-xs hover:bg-muted">
                                                    Copy
                                                </button>
                                                <button type="button" className="rounded-md border border-border bg-background px-2 py-1 text-xs hover:bg-muted">
                                                    Try again
                    ...
                    <TabsContent value="customer-feedback" className="-mt-5">
                        <Card className="rounded-md border border-border p-8 text-center">
                            <MessageCircle className="mx-auto mb-4 size-12 text-[#FBBF24]" aria-hidden />
                            <h3 className="text-xl font-semibold text-foreground">Customer Feedback</h3>
                    ...
                    <TabsContent value="roadmap-planning" className="-mt-5">
                        <Card className="rounded-md border border-border p-8 text-center">
                            <Map className="mx-auto mb-4 size-12 text-[#EA4335]" aria-hidden />
                            <h3 className="text-xl font-semibold text-foreground">Roadmap planning</h3>
                            <p className="mt-2 text-muted-foreground">
                    ...
                    <TabsContent value="project-tracking" className="-mt-5">
                        <Card className="rounded-md border border-border p-8 text-center">
                            <Bug className="mx-auto mb-4 size-12 text-[#673AB7]" aria-hidden />
                            <h3 className="text-xl font-semibold text-foreground">Project and issue tracking</h3>
                            <p className="mt-2 text-muted-foreground">
