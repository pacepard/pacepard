...
import { ITeamDoc, TeamMemberRole } from '../../src/modules/team/team.interface';
 ...
 */
export const createTeamData = (options: TeamFactoryOptions = {}): Partial<ITeamDoc> => {
  const {
  ...
  const workspaceValue = typeof workspace === 'string' 
    ? new Types.ObjectId(workspace) 
    : workspace;
  const businessValue = typeof business === 'string' 
    ? new Types.ObjectId(business) 
    : business;
  const projectValue = typeof project === 'string' 
  ...
  if (!workspaceValue || !businessValue || !projectValue) {
    throw new Error('Workspace, Business, and Project are required for team creation');
  }
 ...
 */
export const createTeam = async (options: TeamFactoryOptions = {}): Promise<ITeamDoc> => {
