...
import { IUserDoc, UserType, PasswordType, OnboardStatus, InviteStatus } from '../../src/modules/user/user.interface';
 ...
 */
export const createUserData = (options: UserFactoryOptions = {}): Partial<IUserDoc> => {
  const {
 ...
 */
export const createUser = async (options: UserFactoryOptions = {}): Promise<IUserDoc> => {
  const userData = createUserData(options);
  const user = await User.create(userData);
  
  // Encrypt password if provided
  if (userData.password && userData.password !== user.password) {
    await authService.encryptUserPassword(user, userData.password as string);
    await user.save();
  }
