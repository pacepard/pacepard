...
import { IProjectDoc, ProjectType, ProjectStatus, ProjectMemberRole } from '../../src/modules/project/project.interface';
 ...
 */
export const createProjectData = (options: ProjectFactoryOptions = {}): Partial<IProjectDoc> => {
  const {
  ...
  const workspaceId = typeof workspace === 'string' 
    ? (Types.ObjectId.isValid(workspace) ? new Types.ObjectId(workspace) : workspace)
    : (workspace?._id || workspace?.id);
  const businessId = typeof business === 'string'
    ? (Types.ObjectId.isValid(business) ? new Types.ObjectId(business) : business)
    : (business?._id || business?.id);
  const createdById: Types.ObjectId = typeof createdBy === 'string'
    ? (Types.ObjectId.isValid(createdBy) ? new Types.ObjectId(createdBy) : new Types.ObjectId(createdBy))
  ...
  return {
    ...
    items: [],
    documentation: '',
    category: faker.helpers.arrayElement(['web', 'mobile', 'ai', 'blockchain', 'iot']),
    tags: faker.helpers.arrayElements(['react', 'node', 'typescript', 'python'], { min: 1, max: 3 }),
    type,
    status,
    ...
    members: members.map(m => {
      const userId = typeof m.user === 'string' 
        ? (Types.ObjectId.isValid(m.user) ? new Types.ObjectId(m.user) : new Types.ObjectId(m.user))
    ...
    tasks: [],
    publishedAt: status === ProjectStatus.PUBLISHED ? new Date() : undefined,
  };
};
 ...
 */
export const createProject = async (options: ProjectFactoryOptions = {}): Promise<IProjectDoc> => {
