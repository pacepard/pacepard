...
import { ITaskDoc, TaskStatusType, TaskPriorityType } from '../../src/modules/task/task.interface';
 ...
 */
const toObjectId = (id: string | Types.ObjectId | undefined | null): Types.ObjectId | undefined => {
 ...
 */
export const createTaskData = (options: TaskFactoryOptions = {}): Partial<ITaskDoc> => {
  const {
  ...
  const workspaceIdValue = typeof workspaceId === 'string' 
    ? new Types.ObjectId(workspaceId) 
    : extractObjectId(workspaceId);
  const businessIdValue = typeof businessId === 'string' 
  ...
  const teamIdValue = typeof teamId === 'string' 
  ...
  if (!workspaceIdValue || !businessIdValue || !projectIdValue || !teamIdValue) {
    throw new Error('Workspace, Business, Project, and Team are required for task creation');
  }
  if (!createdById) {
  ...
  return {
    ...
    tags: faker.helpers.arrayElements(['bug', 'feature', 'refactor', 'documentation'], { min: 0, max: 2 }),
 ...
 */
export const createTask = async (options: TaskFactoryOptions = {}): Promise<ITaskDoc> => {
