...
import { ISubscriptionDoc, SubscriptionStatus, Currency, BillingFrequency, SubscriberUserType } from '../../src/modules/subscription/subscription.interface';
 ...
 */
export const createSubscriptionData = (options: SubscriptionFactoryOptions = {}): Partial<ISubscriptionDoc> => {
  const {
  ...
  const subscriberIdValue = typeof subscriberId === 'string' 
    ? subscriberId 
  ...
  const defaultDueAt = dueAt || new Date(defaultStartAt.getTime() + (billingFrequency === BillingFrequency.MONTHLY ? 30 * 24 * 60 * 60 * 1000 : 365 * 24 * 60 * 60 * 1000));
 ...
 */
export const createSubscription = async (options: SubscriptionFactoryOptions = {}): Promise<ISubscriptionDoc> => {
