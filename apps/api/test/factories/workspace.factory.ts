 ...
 */
export const createWorkspaceData = (options: WorkspaceFactoryOptions = {}): Partial<IWorkspaceDoc> => {
  const {
  ...
  const createdById = typeof createdBy === 'string' ? createdBy : (createdBy?._id || createdBy?.id);
  ...
  return {
    ...
    members: members.map(m => typeof m === 'string' ? m : (m._id || m.id)),
    invites: [],
    hackathons: [],
    projects: [],
    mentors: [],
 ...
 */
export const createWorkspace = async (options: WorkspaceFactoryOptions = {}): Promise<IWorkspaceDoc> => {
