...
import { resolveUserPermissions, hasPermission } from '../../../src/services/permission.service';
...
describe('Permission Service - Unit Tests', () => {
  ...
  describe('resolveUserPermissions', () => {
    ...
    it('should resolve permissions from user ID string', async () => {
      ...
      const permissions = await resolveUserPermissions(String(testUser._id));
    ...
    it('should cache permissions in Redis', async () => {
      ...
      const permissions2 = await resolveUserPermissions(String(testUser._id));
  ...
  describe('hasPermission', () => {
    ...
    it('should support action wildcard permissions', async () => {
      ...
      const hasProjectRead = await hasPermission(testUser, 'project:read');
    ...
    it('should handle resource ownership checks', async () => {
      ...
      const hasAccessOwn = await hasPermission(testUser, 'project:update', {
        resourceOwnerId: String(testUser._id),
        checkOwnership: true,
      });

      // User doesn't own the resource
      const hasAccessOther = await hasPermission(testUser, 'project:update', {
