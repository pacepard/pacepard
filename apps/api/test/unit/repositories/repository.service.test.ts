...
describe('Repository Service - Unit Tests', () => {
  ...
  describe('findById', () => {
    ...
    it('should populate related fields when requested', async () => {
      const result = await repository.findById(String(testUser._id), true);
  ...
  describe('findByIdOrSlug', () => {
    it('should find by ID when valid ObjectId provided', async () => {
      const result = await repository.findByIdOrSlug(String(testUser._id));
    ...
    it('should find by slug when non-ObjectId string provided', async () => {
      ...
      const result = await repository.findByIdOrSlug('invalid-objectid-format');
  ...
  describe('findOne', () => {
    ...
    it('should return error when document not found', async () => {
      const result = await repository.findOne({ email: 'nonexistent@test.com' });
  ...
  describe('findAll', () => {
    ...
    it('should filter documents by query', async () => {
      const result = await repository.findAll({ userType: testUser.userType });
    ...
    it('should support field selection', async () => {
      const result = await repository.findAll({}, { select: 'email,userType' });
  ...
  describe('update', () => {
    it('should update a document by ID', async () => {
      ...
      const result = await repository.update(String(testUser._id), updateData);
    ...
    it('should return error for non-existent ID', async () => {
      const fakeId = require('mongoose').Types.ObjectId();
      const result = await repository.update(String(fakeId), { firstName: 'Test' });
  ...
  describe('count', () => {
    it('should count documents matching filter', async () => {
      const count = await repository.count({ userType: testUser.userType });
    ...
    it('should return 0 for non-matching filter', async () => {
      const count = await repository.count({ email: 'nonexistent@test.com' });
