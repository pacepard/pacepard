...
const startEmailWorker = async () => {
    ...
    const queue = await BullQueue.addProcessor(emailWorkerConfig, processor as any);

    logger.log({ data: `Email Worker started and listening on: ${queueName} (${jobName})`, label: "worker-init", type: "success" });
