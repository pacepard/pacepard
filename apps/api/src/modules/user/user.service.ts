...
class UserService {
  ...
  public async createUser(data: createUserDTO): Promise<IUserDoc> {
    ...
    const existingUserResult = await userRepository.findOne({ email: email.toLowerCase() });
    ...
    if ( userType === UserType.ADMIN ) {
      if (!createdBy) {
        throw new Error("Admin accounts must be created by an existing admin or super admin");
      }
    ...
    if (!creatorId) {
      ...
      const updateResult = await userRepository.updateUser(String(user._id), { createdBy: user._id });
   ...
   * @returns
   */
  public async createUserProfile(data: createUserProfileDTO): Promise<IUserDoc> {
   ...
   // TALENT CHECK & CREATE
    if (user.userType === UserType.TALENT) {
      ...
      const existingTalentResult = await talentRepository.findOne({ user: user._id });
    ...
    if (user.userType === UserType.BUSINESS) {
      const existingOrgResult = await businessRepository.findOne({ user: user._id });
  ...
  public async createBulkUsers(
    ...
    if (data && data.length > 0) {
      for (let i = 0; i < data.length; i++) {
        ...
        const existResult = await userRepository.findOne({ email: bulk.email });
  ...
  async step2SetBasicInfo(
    ...
    if (!data.firstName || !data.lastName || !data.location.country || !data.timeZone) {
      result.error = true;
  ...
  async step3SetBusinessInfo(
    ...
    const businessResult = await businessRepository.findOne({ user: userId });
    ...
    if (businessResult.error === false && businessResult.data) {
      ...
      const createBusinessResult = await businessRepository.createBusiness({
  ...
  async step4CompleteOnboarding(
    ...
    if (user.userType === UserType.TALENT) {
      ...
      if (businessResult.error || !businessResult.data) {
        result.error = true;
        result.message = 'Business profile not found. Please complete step 3.';
        result.code = 400;
        return result;
      }
    }
    ...
    const redirectUrl = user.userType === UserType.TALENT 
      ? '/dashboard/talent' 
      : '/dashboard/business';
  ...
  async getOnboardingStatus(
    ...
    result.data = {
      ...
      status,
      progress,
      canProceed: status !== OnboardStatus.COMPLETED && currentStep < totalSteps,
      userType: user.userType,
    };
