...
export async function hasPermission(
    ...
    if (options?.resource && options?.resourceType) {
        ...
        switch (options.resourceType) {
            case 'workspace': {
                const memberRole = getWorkspaceMemberRole(
                    user,
                    options.resource,
                );
                if (memberRole) {
                    contextualPerms = getContextualPermissions(
                        'workspace',
                        memberRole,
                    );
                }
            ...
            case 'hackathon': {
                ...
                const memberRole = getHackathonMemberRole(
                    user,
                    options.resource,
                );
                if (memberRole) {
                ...
                if (isHackathonJudge(user, options.resource)) {
                    contextualPerms = [
                        ...contextualPerms,
                        ...getHackathonFunctionalPermissions('JUDGE'),
                    ];
                }
                if (isHackathonMentor(user, options.resource)) {
                    contextualPerms = [
                        ...contextualPerms,
                        ...getHackathonFunctionalPermissions('MENTOR'),
                    ];
                }
                break;
            }
        ...
        if (
            contextualPerms.length > 0 &&
            matchPermissionUtil(requested, contextualPerms)
        ) {
            return true;
        }
    }
