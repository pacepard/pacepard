...
const connectDB = async () => {
    ...
    if (!dataBaseURI) {
        throw new Error(
            'Database URI is not defined for the current environment.',
        );
    }
    ...
    try {
        ...
        console.log(
            colors.cyan.bold.underline(
                `Pacepard database ${process.env.NODE_ENV?.toUpperCase()} connected: ${dbConn.connection.host}`,
            ),
        );
    } catch (error) {
        console.log(
            colors.cyan.bold.underline(
                `Could not connect to Pacepard database: ${error}`,
            ),
        );
        process.exit(1);
    }
