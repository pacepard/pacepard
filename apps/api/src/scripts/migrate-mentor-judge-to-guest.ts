...
import { GuestTypeEnum, MentorContextType } from '../modules/users/guest/guest.interface';
...
async function migrateMentorsToGuests() {
    ...
    for (const mentor of mentors) {
        try {
            ...
            if (existingGuest) {
                console.log(`Guest already exists for mentor ${mentor.email}, skipping...`);
                continue;
            }
            ...
            migrated++;
            console.log(`Migrated mentor ${mentor.email} → Guest (${guest._id})`);
        } catch (error: any) {
            errors++;
            console.error(`Error migrating mentor ${mentor.email}:`, error.message);
        }
    }

    console.log(`Mentor migration complete: ${migrated} migrated, ${errors} errors`);
...
async function migrateJudgesToGuests() {
    ...
    for (const judge of judges) {
        try {
            ...
            if (existingGuest) {
                console.log(`Guest already exists for judge ${judge.email}, skipping...`);
                continue;
            }
            ...
            errors++;
            console.error(`Error migrating judge ${judge.email}:`, error.message);
        }
    }

    console.log(`Judge migration complete: ${migrated} migrated, ${errors} errors`);
    return { migrated, errors };
}
...
async function updateWorkspaceReferences() {
    ...
    if (!Workspace) {
        console.log('Workspace model not found, skipping workspace reference updates');
        return;
    }
    ...
    console.log(`Found ${workspaces.length} workspaces with mentors/judges to update`);
    ...
    for (const workspace of workspaces) {
        try {
            ...
            if (workspace.mentors && Array.isArray(workspace.mentors) && workspace.mentors.length > 0) {
                const mentorUserIds = workspace.mentors.map((m: any) => 
                    typeof m === 'object' ? (m._id || m).toString() : m.toString()
                );
            ...
            if (workspace.judges && Array.isArray(workspace.judges) && workspace.judges.length > 0) {
                const judgeUserIds = workspace.judges.map((j: any) => 
                    typeof j === 'object' ? (j._id || j).toString() : j.toString()
                );
            ...
            errors++;
            console.error(`Error updating workspace ${workspace._id}:`, error.message);
        }
    }

    console.log(`Workspace reference update complete: ${updated} updated, ${errors} errors`);
}
...
async function updateHackathonReferences() {
    ...
    if (!Hackathon) {
        console.log('Hackathon model not found, skipping hackathon reference updates');
        return;
    }
    ...
    console.log(`Found ${hackathons.length} hackathons with mentors/judges to update`);
    ...
    for (const hackathon of hackathons) {
        try {
            ...
            // Update mentors: find Guest records with matching user IDs
            if (hackathon.mentors && Array.isArray(hackathon.mentors) && hackathon.mentors.length > 0) {
                const updatedMentors = await Promise.all(
                    hackathon.mentors.map(async (mentor: any) => {
                        const userId = mentor.user?._id?.toString() || mentor.user?.toString() || mentor.user;
            ...
            // Update judges: find Guest records with matching user IDs
            if (hackathon.judges && Array.isArray(hackathon.judges) && hackathon.judges.length > 0) {
                const updatedJudges = await Promise.all(
                    hackathon.judges.map(async (judge: any) => {
                        const userId = judge.user?._id?.toString() || judge.user?.toString() || judge.user;
            ...
            errors++;
            console.error(`Error updating hackathon ${hackathon._id}:`, error.message);
        }
    }

    console.log(`Hackathon reference update complete: ${updated} updated, ${errors} errors`);
}
...
async function updateEntryReferences() {
    ...
    for (const entry of entries) {
        try {
            if (entry.mentors && Array.isArray(entry.mentors) && entry.mentors.length > 0) {
    ...
    console.log(`Entry reference update complete: ${updated} updated, ${errors} errors`);
}
...
async function main() {
    try {
        ...
        console.log(`Mentors migrated: ${mentorResult.migrated}, errors: ${mentorResult.errors}`);
        console.log(`Judges migrated: ${judgeResult.migrated}, errors: ${judgeResult.errors}`);
