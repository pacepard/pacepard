...
class RepositoryService<T extends Document> {
  ...
  public async findAll(
    ...
    try {
      ...
      const pagination = {
        next: endIndex < total ? { page: pageNum + 1, limit: limitNum } : undefined,
        prev: startIndex > 0 ? { page: pageNum - 1, limit: limitNum } : undefined,
      };
  ...
  public async findOne(
    ...
    let populate: boolean | Array<{ path: string }> | string | PopulateOptions | (string | PopulateOptions)[] | undefined;
  ...
  public async create(data: Partial<T> | any): Promise<IResult> {
    const result: IResult = { error: false, message: "", code: 201, data: {} };
  ...
  public async findByEmail(email: string): Promise<IResult> {
    ...
    try {
      const document = await this.model.findOne({ email } as FilterQuery<T>).lean();
      if (!document) {
        result.error = true;
        result.code = 404;
        result.message = `${this.modelName} not found`;
