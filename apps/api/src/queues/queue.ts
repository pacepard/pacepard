...
import { AddJobsDTO, CreateQueueDTO, CreateWorkerDTO, JobDataDTO } from './queue.dto'
    ...
    public async addProcessor(data: CreateWorkerDTO, callback: (data: Job<JobDataDTO>, done: DoneCallback) => Promise<void>): Promise<Queue> {
        ...
        queue.on('completed', (job) => {
            if (ENV.isDevelopment() || ENV.isStaging()) {
                ...
                logger.log({ data: message, label: "job-queue", type: "success" });
            }
        })
        ...
        queue.on('failed', (job, err) => {
            if (ENV.isDevelopment() || ENV.isStaging()) {
                ...
                logger.log({ data: message, label: "job-queue", type: "error" });
            }
        })
        ...
        queue.on('error', (err) => {
            logger.log({
                data: `Queue '${queue.name}' experienced an error: ${err?.message || err
                    }`,
                label: "job-queue",
                type: "error",
            });
