...
previewRoutes.get('/reports/:template', (req: Request, res: Response, next: NextFunction) => {
    const { template } = req.params;
    if (!template) {
        return res.status(400).json({ error: true, message: 'Template parameter is required' });
    }
    ...
    res.render(templatePath, sampleData, (err: Error | null, html: string | undefined) => {
        if (err) {
            return next(err);
        }
        res.send(html);
...
previewRoutes.get('/legal/:template', (req: Request, res: Response, next: NextFunction) => {
    const { template } = req.params;
    if (!template) {
        return res.status(400).json({ error: true, message: 'Template parameter is required' });
    }
    ...
    res.render(templatePath, data, (err: Error | null, html: string | undefined) => {
        if (err) {
            return next(err);
        }
        res.send(html);
...
previewRoutes.get('/:category/:template', (req: Request, res: Response, next: NextFunction) => {
    const { category, template } = req.params;
    if (!category || !template) {
        return res.status(400).json({ error: true, message: 'Category and template parameters are required' });
    }
