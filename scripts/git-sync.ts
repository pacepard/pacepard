...
function checkNodeModulesTracked(): boolean {
...
function step1_CommitNodeModulesRemoval() {
    ...
    if (!checkGitignore()) {
        console.error(
            '❌ node_modules is not in .gitignore. Please add it first.',
        );
        return false;
    }
    ...
    exec(
        'git rm -r --cached node_modules apps/*/node_modules packages/*/node_modules 2>/dev/null || true',
        { stdio: 'inherit' },
    );
    exec('git add -A', { stdio: 'inherit' });
    exec(`git commit -m "Remove node_modules from git tracking"`, {
        stdio: 'inherit',
    });
...
function step3_Push() {
    ...
    try {
        ...
        console.error(
            '❌ Push failed. Check your permissions and network connection.',
        );
        return false;
    }
}
